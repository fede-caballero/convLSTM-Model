...
input_root_dir = "/home/f-caballero/UM/TIF3/convLSTM-project/convLSTM-Model/netCDF_samples/netCDF_Big_sample/netcdf_500x500x18/20111115"
...
MIN_RELEVANT_DBZ = 5.0  # UMBRAL FÍSICO: Solo nos interesan datos por encima de este valor.
...
for nc_file_in_path in all_files:
    ...
    try:
        with nc.Dataset(nc_file_in_path, 'r') as ds_in:
            ...
            # --- 1. LECTURA Y "LAVADO" INMEDIATO DEL ARRAY ---
            dbz_var_in = ds_in.variables['DBZ']
            dbz_var_in.set_auto_mask(False)
            dbz_var_in.set_auto_scale(False)
            
            # Se lee y se fuerza la creación de una copia de numpy pura.
            # Esta es la línea que resuelve la anomalía de la librería netCDF4.
            data_raw = np.array(dbz_var_in[:], copy=True)
            
            # --- 2. DESEMPAQUETADO SEGURO ---
            # Ahora se opera sobre 'data_raw', un array de numpy estándar.
            ...
            if fill_value_packed_in is not None:
                # Esta comparación ahora funcionará correctamente.
                dbz_physical[data_raw == fill_value_packed_in] = np.nan
            
            dbz_working = dbz_physical[0].copy()

            # --- 3. LIMPIEZA POR RELEVANCIA FÍSICA ---
            # Se crea una máscara para TODO lo que no queremos (ruido, nulos, etc.)
            irrelevant_data_mask = (dbz_working < MIN_RELEVANT_DBZ) | np.isnan(dbz_working)
            # Se aplica la máscara, convirtiendo todo lo irrelevante en NaN
            dbz_working[irrelevant_data_mask] = np.nan

            # --- 4. REDIMENSIONAMIENTO SEGURO (si es necesario) ---
            ...
            # --- 5. EMPAQUETADO FINAL Y CORRECTO ---
            # Primero, se clipean los datos. np.clip ignora los NaN, por lo que los nulos no se ven afectados.
            dbz_clipped = np.clip(dbz_final_physical, CLIP_PHYSICAL_MIN_DBZ, CLIP_PHYSICAL_MAX_DBZ)
            
            # Ahora, se reemplazan los NaN con el valor físico de relleno.
            dbz_for_packing = np.where(np.isnan(dbz_clipped), PHYSICAL_FILL_VALUE, dbz_clipped)
            
            # Se calcula el valor de byte.
            dbz_packed_byte = np.round((dbz_for_packing - OUTPUT_NC_ADD_OFFSET) / OUTPUT_NC_SCALE_FACTOR).astype(np.int8)
            
            # Se asegura que el valor físico de relleno corresponda exactamente al byte de relleno.
            ...
            # --- 6. ESCRITURA DEL ARCHIVO NETCDF ---
            with nc.Dataset(output_file_path, 'w', format='NETCDF3_CLASSIC') as ds_out:
                # La lógica de escritura que ya tenías es correcta.
